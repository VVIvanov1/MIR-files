function getFareValue(text, ...args) {
    ...
    let patternFare = /(^A07[\s\S]*?^)A08/gm

    let res = patternFare.exec(text)
    let pieceLength = res[1].length / count
    let chunks = []
    let start = 0
    for (let i = 0; i < count; i++) {
        chunks.push(res[1].substr(start, pieceLength - 1))
...
function getItTaxBreakdown(input) {
    ...
    if (input.indexOf('XT') !== -1) {
        let itTax = input.slice(input.indexOf('IT:')).replace("\r", "").replace("\n", "")
